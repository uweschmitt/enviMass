...
plot_measurements<-function(
	...
	mtext("doted",side=2,at=1,las=1,col="blue",cex=1)
	mtext("calibration",side=2,at=2,las=1,col="red",cex=1)
	mtext("blank/blind",side=2,at=3,las=1,col="orange",cex=1)
	mtext("sample",side=2,at=4,las=1,col="darkgreen",cex=1)
	mtext("doted",side=2,at=7,las=1,col="blue",cex=1)
	...
	if(any((measurements$Type=="calibration")&(measurements$Mode=="positive"))){ # add bars for calibration groups, positive!
		...
		for(i in 1:length(measurements2[,1])){
			...
			lines(y=c(8,8),x=c(atPOSIX_start,atPOSIX_end),col="red")
			lines(y=c(7.8,8.2),x=c(atPOSIX_start,atPOSIX_start),col="red")
			lines(y=c(7.8,8.2),x=c(atPOSIX_end,atPOSIX_end),col="red")
		}
	}
	if(any((measurements$Type=="calibration")&(measurements$Mode=="negative"))){ # add bars for calibration groups, negative!
		...
		for(i in 1:length(measurements2[,1])){
			...
			end_datetime<-paste(end_dated,end_timed,"CET",sep=" ")
			atPOSIX_end<-as.POSIXct(end_datetime)
			lines(y=c(2,2),x=c(atPOSIX_start,atPOSIX_end),col="red")
			lines(y=c(1.8,2.2),x=c(atPOSIX_start,atPOSIX_start),col="red")
			lines(y=c(1.8,2.2),x=c(atPOSIX_end,atPOSIX_end),col="red")
		}
	}
	...
	for(i in 1:length(measurements[,1])){
		...
		if(measurements[i,]$Mode=="positive"){
			if(measurements[i,]$Type=="sample"){at_y<-10;use_col<-"darkgreen";with_pch<-15}
			if(measurements[i,]$Type=="blank"){at_y<-9;use_col<-"orange";with_pch<-15}		
			if(measurements[i,]$Type=="calibration"){at_y<-8;use_col<-"black";with_pch<-0}		
			if(measurements[i,]$Type=="doted"){at_y<-7;use_col<-"blue";with_pch<-15}		
		}else{
			if(measurements[i,]$Type=="sample"){at_y<-4;use_col<-"darkgreen";with_pch<-15}		
			if(measurements[i,]$Type=="blank"){at_y<-3;use_col<-"orange";with_pch<-15}	
			if(measurements[i,]$Type=="calibration"){at_y<-2;use_col<-"black";with_pch<-0}			
			if(measurements[i,]$Type=="doted"){at_y<-1;use_col<-"blue";with_pch<-15}		
		}

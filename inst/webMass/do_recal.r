    ...
    measurements_incl<-measurements[measurements[,8]=="TRUE",]
	...
	mz_neg<-c();
	RT_neg<-c();
    if(any(measurements_incl[,4]=="positive")){ # positive ##########################
    ...
    if(any(measurements_incl[,4]=="negative")){ # negative ##########################
      if(logfile$parameters[30]=="Internal standards"){
    ...
    for( i in 1:length(measurements_incl[,1]) ){  
		if( (measurements_incl[i,8]=="TRUE")&(measurements_incl[i,12]=="FALSE")  ){	  
		  if(any(objects(envir=as.environment(".GlobalEnv"))=="peaklist")){rm(peaklist,envir=as.environment(".GlobalEnv"))}
		  if(any(objects()=="peaklist")){rm(peaklist)}
		  load(file=file.path(logfile[[1]],"peaklist","",as.character(measurements_incl[i,1])),envir=as.environment(".GlobalEnv"));   
			...
			if( length(mz_pos)>0 ){
				  peak_recal<-recalib(
					...
					if(length(peak_recal)>1){
					  ...
					  save(peaklist,file=file.path(logfile[[1]],"peaklist",as.character(measurements_incl[i,1])));   
					  measurements_incl[i,12]<-"FALSE";
					...
					measurements[
						measurements[,1]==measurements_incl[i,1]
					,12]<-TRUE;
			}else{		
				png(filename = file.path(logfile[[1]],"pics",paste("recal_",as.character(measurements_incl[i,1]),sep="")), bg = "white", width = 1100, height= 300)
				plot.new()

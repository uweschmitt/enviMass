		...
		if(length(cal_models_neg_used)>0){ # no calibration models? 
			...
			if(length(res_target_neg_screen)>0){
				for(i in 1:length(res_target_neg_screen)){
					if(length(res_target_neg_screen[[i]])>0){ # anything screened?
						...
						if(target_table[target_table[,1]==at_ID,6]!="FALSE"){ # target linked to an IS compound?
							...
							for(j in 1:length(res_target_neg_screen[[i]])){ # over files
								if(length(res_target_neg_screen[[i]][[j]])>0){ # at this file, target
									...
									if(length(at_group)==0){next} # calibration group available?
									at_group_model<-which(names(cal_models_neg_used[[at_group]])==paste(at_IS,at_ID,sep="_"))					
									if(length(at_group_model)==0){next} # model in that group available?
									at_adduct_IS<-IS_table[IS_table[,1]==at_IS,19] 	# get relevant IS adduct
									at_peak_IS<-IS_table[IS_table[,1]==at_IS,20] 	# get relevant IS peak
									get_conc<-c()
									for(k in 1:length(res_target_neg_screen[[i]][[j]])){ # all matches for this file
										...
										if(length(use_target_peak)>0){
											...
											high_bound<-as.numeric(IS_table[IS_table[,1]==at_IS,18])
											if(high_bound!=0){high_bound<-(10^high_bound)}
											for(w in 1:length(res_IS_neg_screen[[at_IS_entry]][[at_IS_entry_sample]])){
												# check IS peak
											...
											if(length(get_conc)==0){next}
											res_target_neg_screen[[i]][[j]][[k]]$conc<-get_conc
											found_which[[length(found_which)+1]]<-c(i,j,k)
									...
									target_quant_table_neg[
										rownames(target_quant_table_neg)==at_ID,
										colnames(target_quant_table_neg)==at_sample
									]<-get_conc
								}						
							}
						}

...
######################################################################################
	...
	for(b in 1:length(measurements[,"ID"])){
		if( 
			...
			(measurements[b,"EIC_correlation"]=="FALSE")  	# not yet done
		){ 
			...
			for(i in 1:length(paired[,1])){			
				...
				#if(del_1<as.numeric(logfile$parameters$EICor_minpeaks)){ # not enough EIC signals, to be removed later
				#	paired[i,4]<-1000
				#	next;
				#}
				...
				#if(del_2<as.numeric(logfile$parameters$EICor_minpeaks)){ # not enough EIC signals, to be removed later
				#	paired[i,4]<-1000
				#	next;
				#}				
				####################################################################

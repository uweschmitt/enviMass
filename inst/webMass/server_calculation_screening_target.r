...
if(  
	logfile$workflow[names(logfile$workflow)=="screen_target"]=="yes" && 
	!(
		(logfile$summary[(logfile$summary[,1]=="Target screening"),2]=="TRUE") &&
		(logfile$Tasks_to_redo[names(logfile$Tasks_to_redo)=="target_screen"]=="FALSE")
	)
){
...
dev.off()
	...
	summa[(logfile$summary[,1]=="Target screening"),2]<<-"done"
	output$summa_html<<-renderText(summary_html(summa));
    cat("Target screening done done \n");
    output$dowhat<<-renderText("Target screening done ... wait")	
	
}else{

	if((logfile$workflow[names(logfile$workflow)=="screen_target"]=="no")){
		if(
			!(
				(logfile$summary[(logfile$summary[,1]=="Target screening"),2]=="TRUE") &
				(logfile$Tasks_to_redo[names(logfile$Tasks_to_redo)=="target_screen"]=="FALSE")
		...
		logfile$summary[(logfile$summary[,1]=="Target screening"),2]<<-"FALSE";
		logfile$summary[(logfile$summary[,1]=="Target screening"),2]<-"FALSE";	
		logfile$Tasks_to_redo[names(logfile$Tasks_to_redo)=="target_screen"]<-"FALSE";
		logfile$Tasks_to_redo[names(logfile$Tasks_to_redo)=="target_screen"]<<-"FALSE";
		save(logfile,file=file.path(as.character(logfile[[1]]),"logfile.emp"));
	}
    summa[(logfile$summary[,1]=="Target screening"),2]<-"skipped"
	summa[(logfile$summary[,1]=="Target screening"),2]<<-"skipped"
	output$summa_html<<-renderText(summary_html(summa));

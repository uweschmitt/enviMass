	...
	if( length(links_peaks_pos)>0 ){ # if any links exist
		...
		for(i in 1:length(profileList_pos[[2]][,1])){
			if(profileList_pos[[2]][i,5]!=0){
				if(length(links_peaks_pos[[profileList_pos[[2]][i,5]]][[1]])>0){
					keep_peaks[i]<-FALSE
					links_peaks_pos[[profileList_pos[[2]][i,5]]][[1]]<-list() # = target entry[[1]] in links_peaks_pos
				}
			}
		}
		...
		cat(paste("\nTarget subtraction: ",round((sum(!keep_peaks)/length(keep_peaks)*100),digits=3),"% of peaks removed",sep=""))
		###############################################################################################
		save(profileList_pos,file=file.path(as.character(logfile[[1]]),"results","profileList_pos"))
	}
}
		...
		for(i in 1:length(profileList_neg[[2]][,1])){
			if(profileList_neg[[2]][i,5]!=0){
				if(length(links_peaks_neg[[profileList_neg[[2]][i,5]]][[1]])>0){
					keep_peaks[i]<-FALSE
					links_peaks_neg[[profileList_neg[[2]][i,5]]][[1]]<-list() # = target entry[[1]] in links_peaks_pos
				}
			}
		}
